...
public class SqliteDaoImpl<T extends DataBaseModel> extends Dao<T> {
    ...
    public SqliteDaoImpl(String dbUrl, boolean showSql) throws Exception {
        super(dbUrl, null, DriverName.SqliteDriver, null, null, false, showSql);
//        new File(dbName).
            ...
            if (columns != null && !columns.isEmpty()) {
                for (SqlColumn column : columns) {
                    if (existsColumn(con, tableName, column.getColumnName())) {
                        if (this.isShowSql()) {
                            log.error("数据库：" + this.getDbUrl() + " 表：" + tableName + " 映射数据库字段：" + column.getColumnName() + " 检查结果：已存在，将不会修改");
                        } else {
    ...
    public int createDatabase(String database) throws Exception {
        throw new UnsupportedOperationException("Create Database do not Operation by sqlite");
    }

    @Override
    public int dropDatabase(String database) throws Exception {
        throw new UnsupportedOperationException("Drop Database do not Operation by sqlite");
    }
