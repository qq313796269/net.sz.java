 ...
 */
public class CUDThread extends BaseThread {
    ...
    public CUDThread(Dao dao, String name) {
        ...
        this.dao = dao;
        this.insertDbObjects = new DbObjects();
        this.updateDbObjects = new DbObjects();
        this.deleteDbObjects = new DbObjects();

        this.insertDbRuns = new DbRuns();
        this.updateDbRuns = new DbRuns();
        this.deleteDbRuns = new DbRuns();
        this.maxTaskCount = 50000;
        this.start();
    }
                    ...
                    if (!objects.isEmpty()) {
                        try {
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug(this.getName() + " 删除数据影响行数：" + deleteList + " 耗时：" + (TimeUtil.currentTimeMillis() - currentTimeMillis));
                            }
                        } catch (Throwable ex) {
                            log.error(this.getName() + " 异步写入数据库错误 删除对象", ex);
                        }
                    }
                ...
                }
                {
                    ...
                    if (!objects.isEmpty()) {
                        try {
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug(this.getName() + " 新增数据插入影响行数：" + insertList + " 耗时：" + (TimeUtil.currentTimeMillis() - currentTimeMillis));
                            }
                        } catch (Throwable ex) {
                            log.error(this.getName() + " 异步写入数据库错误 插入对象", ex);
                        }
                    }
                ...
                }
                {
                    ...
                    if (!objects.isEmpty()) {
                        try {
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug(this.getName() + " 更新数据插入影响行数：" + updateList + " 耗时：" + (TimeUtil.currentTimeMillis() - currentTimeMillis));
                            }
                        } catch (Throwable ex) {
                            log.error(this.getName() + " 异步写入数据库错误 更新对象", ex);
                        }
                    }
                    CUDThread.this.updateDbObjects.values.clear();
                }
            } catch (Throwable ex) {
                log.error(this.getName() + " 数据库错误", ex);
            }
            if (log.isDebugEnabled()) {
                log.debug(this.getName() + " 当前待处理剩余数量：" + size());
            }
        }
    }
    ...
    final class DbRuns {
        ...
        boolean add(IDaoRun run) {
            if (size() > getMaxTaskCount()) {
                throw new UnsupportedOperationException(CUDThread.this.getName() + " 积压过多的任务：" + getMaxTaskCount());
            }
            return objs.add(run);
        }
