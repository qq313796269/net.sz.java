...
import net.sz.framework.utils.FileUtil;
...
public final class ScriptPool {
    ...
    public static HashMap<String, Long> cacheClassLastModifyTimeMap = new HashMap<String, Long>();
...
//        }
    ...
    ConcurrentHashMap<Serializable, IBaseScript> script2ScriptKeyInstances = new ConcurrentHashMap<>(0);
    ConcurrentHashMap<Serializable, IBaseScript> tmpScript2ScriptKeyInstances = new ConcurrentHashMap<>(0);
    ...
    static final ArrayList NULL_ARRAYLIST_EVT = new ArrayList<>();
     ...
     * @return
     */
    public IBaseScript getEvt(Serializable key) {
        return ScriptPool.this.script2ScriptKeyInstances.get(key);
    }

    public <T extends IBaseScript> T getEvt(Serializable key, Class<T> t) {
        return (T) ScriptPool.this.script2ScriptKeyInstances.get(key);
    }
...
//                    options.add("unchecked");
                ...
                if (!call) {
                    for (Diagnostic oDiagnostic : oDiagnosticCollector.getDiagnostics()) {
                        if (oDiagnostic.getKind() == Diagnostic.Kind.ERROR || oDiagnostic.getKind() == Diagnostic.Kind.MANDATORY_WARNING) {
                            String error = "脚本错误：" + oDiagnostic.getMessage(Locale.SIMPLIFIED_CHINESE) + " 文件：" + ((JavaFileObject) (oDiagnostic.getSource())).getName() + " line:" + oDiagnostic.getLineNumber() + " pos:" + oDiagnostic.getStartPosition();
                            strs.add(error);
                            log.error(error);
                        }
                    }
    ...
    private void getFiles(File sourceFile, List<File> sourceFileList, final String endName) {
        if (sourceFile.exists() && sourceFileList != null) {// 文件或者目录必须存在
            if (sourceFile.isDirectory()) {// 若file对象为目录
                ...
                /*若file对象为文件*/
                String replaceAll = sourceFile.getPath().substring(sourceDir.length());
                Long get = cacheClassLastModifyTimeMap.get(sourceFile.getPath());
                if (get == null || get != sourceFile.lastModified()) {
                    log.error("找到脚本文件：" + replaceAll);
                    sourceFileList.add(sourceFile);
                    cacheClassLastModifyTimeMap.put(sourceFile.getPath(), sourceFile.lastModified());
                } else {
                    log.error("脚本文件：" + replaceAll + " 并无修改不会再次加载");
                }
            }
        }
    }
     ...
     * @return
     */
    public ArrayList<String> loadJavas() {
        synchronized (this) {
            ...
            if (compile == null || compile.isEmpty()) {
                ...
                tmpScript2ScriptKeyInstances = new ConcurrentHashMap<>();
                ...
                if (tmpScript2ScriptKeyInstances.size() > 0) {
                    script2ScriptKeyInstances = tmpScript2ScriptKeyInstances;
                }
            ...
            if (compile == null || compile.isEmpty()) {
                ...
                if (source == null || source.length <= 0) {
                    ...
                    for (String string : source) {
                        ...
                        if (pathString.endsWith(".java")) {
                            pathString = pathString.replace(".java", ".class");
                        }
                        getFiles(this.outDir + pathString, sourceFileList, ".class");
                    }
                }
                String[] fileNames = new String[sourceFileList.size()];
                for (int i = 0; i < sourceFileList.size(); i++) {
                ...
                if (tmpScriptInstances.size() > 0) {
                    for (Map.Entry<String, ConcurrentHashMap<String, IBaseScript>> entry : tmpScriptInstances.entrySet()) {
                        ConcurrentHashMap<String, IBaseScript> get = scriptInstances.get(entry.getKey());
                        if (get == null) {
                            scriptInstances.put(entry.getKey(), entry.getValue());
                        } else {
                            get.putAll(entry.getValue());
                        }
                    }
                }

                if (tmpScript2ScriptKeyInstances.size() > 0) {
                    for (Map.Entry<Serializable, IBaseScript> entry : tmpScript2ScriptKeyInstances.entrySet()) {
                        Serializable key = entry.getKey();
                        IBaseScript value = entry.getValue();
                        script2ScriptKeyInstances.put(key, value);
                    }
                }
...
//        }
            ...
            if (classData != null) {
                try {
                    ...
                    if (defineClass.isInterface()
                            || Modifier.isAbstract(defineClass.getModifiers())
                        ...
                        if (newInstance != null) {
                            ...
                            for (Class<?> aInterface : interfaces) {
                                ...
                                if (IBaseScript.class.isAssignableFrom(aInterface)) {
                                    ...
                                    IBaseScript iBaseScript = (IBaseScript) newInstance;
                                    Serializable scriptKey = iBaseScript.getScriptKey();
                                    if (scriptKey != null) {
                                        tmpScript2ScriptKeyInstances.put(scriptKey, iBaseScript);
                                    }
                                    if (!tmpScriptInstances.containsKey(aInterface.getName())) {
                                        tmpScriptInstances.put(aInterface.getName(), new ConcurrentHashMap<>());
                                    }
                                    tmpScriptInstances.get(aInterface.getName()).put(defineClass.getName(), iBaseScript);
                                }
                            }
        ...
        private byte[] getClassData(String className) {
            ...
            try {
                ...
                if (file.exists()) {
                    return FileUtil.readFileToBytes(path);
                } else {
